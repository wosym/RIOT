...
static void _can_event_callback(candev_t *candev, candev_event_t event, void *arg)
{
    (void) arg;
    ...
    case CANDEV_EVENT_RX_INDICATION:
        ...
        isrpipe_write_one(&rxbuf, (uint8_t)((frame->can_id & 0x1FFFFFFF) >> 6));    //exclude flags
        isrpipe_write_one(&rxbuf, (uint8_t)((frame->can_id & 0xFF0000) >> 4));
    ...
    case CANDEV_EVENT_BUS_OFF:
        candev->state = CAN_STATE_BUS_OFF;
        break;
    case CANDEV_EVENT_ERROR_PASSIVE:
        candev->state = CAN_STATE_ERROR_PASSIVE;
        break;
    case CANDEV_EVENT_ERROR_WARNING:
        candev->state = CAN_STATE_ERROR_WARNING;
        break;
    default:
...
static int _send(int argc, char **argv)
{
    ...
    if(argc > 1)
    {
        if(argc > 1 + CAN_MAX_DLEN){
            printf("Could net send. Maximum CAN-bytes: %d\n", CAN_MAX_DLEN);
            return -1;
        }
    ...
    ret = dev.candev.driver->send(&dev.candev, &frame);
...
static int _receive(int argc, char **argv)
{
    ...
    for(int i = 0; i < n; i++) {
        ...
        puts("Reading from Rxbuf..."); isrpipe_read(&rxbuf, buf, 4);  //id
...
int main(void)
{
    ...
    dev.candev.event_callback = _can_event_callback;
    dev.candev.driver->init(&(dev.candev));
